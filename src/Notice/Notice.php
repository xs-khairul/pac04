...
?>
        <div id="<?php echo esc_attr($this->notice_id); ?>" class="notice xpeedstudio-notice notice-<?php echo esc_attr($this->notice_id . ' ' . $this->class); ?> <?php echo (false === $this->dismissible ? '' : 'is-dismissible'); ?>" expired_time="<?php echo esc_attr($this->expired_time); ?>" dismissible="<?php echo esc_attr($this->dismissible); ?>">
                    ...
                    <?php if (!empty($this->buttons)) : ?>
                        <div class="button-container notice-vert-space">
                            <?php foreach ($this->buttons as $button) : ?>
                                <a id="<?php echo (!isset($button['id']) ? '' : esc_attr($button['id'])); ?>" href="<?php echo esc_url($button['url']); ?>" class="xpeedstudio-notice-button <?php echo esc_attr($button['class']); ?>">
...
<?php
    ...
    {
        add_action('wp_ajax_xpeedstudio-notices', array(__CLASS__, 'dismiss_ajax_call'));
    ...
    public static function dismiss_ajax_call()
    {
        ...
        if (empty($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'xpeedstudio-notices')) {
            return false;
        }
    ...
    public static function enqueue_scripts()
    {
        ...
        <script>
            jQuery(document).ready(function ($) {
                $( '.xpeedstudio-notice.is-dismissible' ).on( 'click', '.notice-dismiss', function() {

                    _this 		        = $( this ).parents('.xpeedstudio-notice').eq(0);
                    ...
                    $.ajax({
                        ...
                        type: 'POST',
                        data: {
                            action 	        : 'xpeedstudio-notices',
                            notice_id 		: notice_id,
                            dismissible 	: dismissible,
                            expired_time 	: expired_time,
                            nonce 			: '" . esc_js(wp_create_nonce('xpeedstudio-notices')) . "'
                        },
                    });
                });
            });
        ...
        <style>
            ...
            .xpeedstudio-notice .notice-right-container{
            ...
            .xpeedstudio-notice.no-gutter{
                padding: 0!important;
                border-width: 0!important;
            }
            .xpeedstudio-notice.no-gutter .notice-right-container{
                padding: 0!important;
                margin: 0!important;
            }
            ...
            .xpeedstudio-notice .notice-logo{
                padding: 3px;
                max-width: 110px;
                max-height: 110px;
            }
            
            .xpeedstudio-notice-button {
                text-decoration:none;
            }
            
            .xpeedstudio-notice-button > i{
                margin-right: 3px;
            }
            
            .xpeedstudio-notice-button .notice-icon{
                display:inline-block;
            }

            .xpeedstudio-notice-button .notice-icon:before{
                vertical-align: middle!important;
                margin-top: -1px;
            }

            .xpeedstudio-notice .notice-main-title{
                color: #1d2327;
                font-size: 1.2rem;
            }
